...
4. 「**+ Create Cluster**」をクリックして、新しいクラスターを追加します。

    ![「クラスターの作成」ページ](images/create-a-cluster.png)

5. `Test Cluster` など、クラスターの名前を入力します。
6. [**Cluster Mode**] を **Single Node** に変更します。
...
10. 次のタスクに進む前にクラスターが開始されるまで待ちます。クラスターが起動するまで 5 〜 7 分待つ必要があることに注意してください。

### タスク 2: Databricks アーカイブを複製する

1. Azure Databricks ワークスペースの左側のウィンドウで、「**Workspace**」 > 「**Users**」を選択し、ユーザー名 (家のアイコンが付いたエントリ) を選択します。
1. 表示されたウィンドウで、ご自分の名前の横にある矢印を選択し、「**Import**」を選択します。
...
1. 上部にある **Detached** と表示されているドロップダウンの値を変更して先の手順で作成したクラスターに接続します。コードが入力されている Cmdセルを順番に実行します。実行するには、実行するセルを選択してから、右上にある **&#x23f5;Run Cell** ボタンを使用して実行します。
